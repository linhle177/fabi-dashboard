...
st.set_page_config(page_title="Fabi Dashboard", page_icon="☕", layout="wide", initial_sidebar_state="expanded")

# ── COLORS (light theme như ảnh ref) ──────────────────
BG       = "#F5F3EF"   # nền kem
CARD     = "#FFFFFF"   # card trắng
CARD_ACC = "#1A1A1A"   # card accent đen (doanh thu)
BORDER   = "#E8E4DC"
TEXT     = "#1A1A1A"
SUBTEXT  = "#888888"
NAVY     = "#1D3557"   # bar accent
NAVY_LT  = "#E8EDF3"   # bar default
GOLD     = "#F5C518"
GREEN    = "#2ECC71"
...
[data-testid="stSidebar"] section {{ padding: 24px 20px; }}
[data-testid="stFileUploader"] {{
    background: {BG} !important;
    border: 2px dashed {BORDER} !important;
    border-radius: 10px !important;
}}
[data-testid="stFileUploader"] * {{ color: {SUBTEXT} !important; }}
.stButton > button {{
...
.m-card.dark .lbl {{ color: rgba(255,255,255,.5); }}
.m-card .val {{
    font-size: 32px;
    font-weight: 800;
    color: {TEXT};
...
.m-card.dark .val {{ color: #fff; }}
.m-card .sub {{
    font-size: 12px;
    color: {SUBTEXT};
}}
.m-card.dark .sub {{ color: rgba(255,255,255,.45); }}
...
.ft th {{
    ...
    letter-spacing:.6px; color:{SUBTEXT};
    padding: 6px 10px; border-bottom: 1.5px solid {BORDER};
    text-align:left;
}}
...
.ft .g {{ color:{SUBTEXT}; font-size:11px; }}
...
.hdr-logo {{ font-size:22px; font-weight:900; color:{TEXT}; letter-spacing:-1px; }}
.hdr-sub {{ font-size:12px; color:{SUBTEXT}; margin-top:1px; }}
.hdr-period {{
...
.pill {{
    ...
    background:{CARD}; color:{SUBTEXT};
    cursor:pointer;
}}
.pill.on {{ background:{TEXT}; color:#fff; border-color:{TEXT}; font-weight:600; }}
</style>
...
def parse_fabi(fb: bytes):
    ...
    while i < len(rows):
        ...
        if len(cells) == 40 and cells[0].isdigit():
            inv = dict(ngay=cells[6], gio=cells[7], ban=cells[11],
            ...
            if dt:
                inv.update(dt=dt, date_only=dt.date(),
                           ...
                           thang=dt.strftime("%m/%Y"), hour=dt.hour)
            invs.append(inv)
            i = j
        else: i+=1
    return invs
...
CL = dict(
    ...
    xaxis=dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=11)),
    yaxis=dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=11)),
)
...
with st.sidebar:
    ...
    <div style='padding:0 0 18px'>
        <div style='font-size:20px;font-weight:900;color:{TEXT}'>☕ Fabi</div>
        <div style='font-size:11px;color:{SUBTEXT};margin-top:1px'>Sales Dashboard</div>
    </div>""", unsafe_allow_html=True)

    uploaded = st.file_uploader("Tải file XLS từ Fabi", type=["xls","xlsx"],
    ...
    st.markdown(f"<div style='font-size:12px;font-weight:600;color:{TEXT};margin-bottom:8px'>Xem theo</div>", unsafe_allow_html=True)
...
if not uploaded:
    ...
    <div style='display:flex;flex-direction:column;align-items:center;justify-content:center;
                min-height:80vh;text-align:center;gap:10px'>
        <div style='font-size:52px'>☕</div>
        <div style='font-size:24px;font-weight:800;color:{TEXT}'>Fabi Sales Dashboard</div>
        <div style='font-size:14px;color:{SUBTEXT};max-width:320px;line-height:1.7'>
            Tải file <b>.xls</b> xuất từ Fabi lên thanh bên trái để bắt đầu
        </div>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
<div class="hdr">
    ...
    <div class="hdr-period">{period_str}</div>
</div>""", unsafe_allow_html=True)
...
for col, lbl, val, sub, dark in defs:
    col.markdown(f"""
    <div class="m-card {'dark' if dark else ''}">
...
tab1,tab2,tab3 = st.tabs(["💰 Doanh thu","📋 Số đơn","📊 AOV theo ngày"])

def bar_chart(x_vals, y_vals, hover_custom, ticksuffix="", color_peak=True, height=260):
    mx = max(y_vals) if y_vals else 1
    colors = [NAVY if (v==mx and color_peak) else NAVY_LT for v in y_vals]
    fig = go.Figure(go.Bar(
        x=x_vals, y=y_vals, marker_color=colors, marker_line_width=0,
        hovertemplate="%{x}<br><b>%{customdata}</b><extra></extra>",
        customdata=hover_custom,
    ))
    fig.update_layout(**CL, height=height, bargap=0.35,
        yaxis=dict(ticksuffix=ticksuffix, gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=11)),
        xaxis=dict(gridcolor="rgba(0,0,0,0)", linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=11)),
    )
...
with tab1:
    ...
    st.plotly_chart(bar_chart(gkeys, y, [ff(rev_g.get(k,0)) for k in gkeys], ticksuffix="k"), use_container_width=True)
with tab2:
    y = [inv_g.get(k,0) for k in gkeys]
    st.plotly_chart(bar_chart(gkeys, y, [f"{v} đơn" for v in y], color_peak=False), use_container_width=True)
with tab3:
    y = [aov_g.get(k,0) for k in gkeys]
    fig3 = go.Figure(go.Scatter(
        x=gkeys, y=y, mode="lines+markers",
        line=dict(color=NAVY, width=2.5), marker=dict(size=7, color=NAVY),
        hovertemplate="%{x}<br>AOV: <b>%{customdata}</b><extra></extra>",
        customdata=[ff(v) for v in y],
    ))
...
with L:
    ...
    t_labels  = [t for t in thu_order if t in aov_thu]
    t_vals    = [sum(aov_thu[t])//len(aov_thu[t]) for t in t_labels]
    mx = max(t_vals) if t_vals else 1
    fig_t = go.Figure(go.Bar(
        x=t_labels, y=t_vals, marker_line_width=0,
        marker_color=[NAVY if v==mx else NAVY_LT for v in t_vals],
        hovertemplate="%{x}<br>AOV: <b>%{customdata}</b><extra></extra>",
        customdata=[ff(v) for v in t_vals],
    ))
    fig_t.update_layout(**CL, height=240, bargap=0.4,
        title=dict(text="AOV theo thứ trong tuần", font=dict(size=13,color=TEXT), x=0),
        yaxis=dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=10)),
        xaxis=dict(gridcolor="rgba(0,0,0,0)", linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=10)),
    )
...
with R:
    ...
    fig_h.update_layout(**CL, height=300, bargap=0.3,
        title=dict(text="Top 10 món bán chạy", font=dict(size=13,color=TEXT), x=0),
        xaxis=dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=10)),
...
with c1:
    ...
    fig_pie = go.Figure(go.Pie(
        ...
        hole=0.58, marker_colors=[NAVY_LT, NAVY, GOLD],
        textfont=dict(color=TEXT,size=11),
        hovertemplate="%{label}: <b>%{value} đơn</b> (%{percent})<extra></extra>",
    ))
    fig_pie.update_layout(**CL, height=240,
...
with c2:
    ...
    fig_tp.update_layout(**CL, height=240, bargap=0.35,
        title=dict(text="% đơn nhóm theo thứ", font=dict(size=13,color=TEXT), x=0),
        yaxis=dict(range=[0,100], ticksuffix="%", gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=10)),
        xaxis=dict(gridcolor="rgba(0,0,0,0)", linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=10)),
    )
...
with c3:
    ...
    fig_hr.update_layout(**CL, height=240, bargap=0.2,
        title=dict(text=f"Giờ cao điểm (peak {peak:02d}:00)", font=dict(size=13,color=TEXT), x=0),
        xaxis=dict(gridcolor="rgba(0,0,0,0)", linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=9)),
        yaxis=dict(gridcolor=BORDER, linecolor=BORDER, tickfont=dict(color=SUBTEXT,size=10)),
    )
...
with tl:
    ...
    st.markdown(f"""
    <div style='font-size:13px;font-weight:600;color:{TEXT};margin-bottom:12px'>Top 10 món bán chạy</div>
    <table class='ft'>
...
with tr2:
    ...
    fig_pt = go.Figure(go.Pie(
        ...
        values=[tr_r, cd_r], hole=0.55,
        marker_colors=[NAVY, NAVY_LT],
        textfont=dict(color=TEXT, size=12),
        hovertemplate="%{label}<br><b>%{customdata}</b><extra></extra>",
        customdata=[ff(tr_r), ff(cd_r)],
    ))
    fig_pt.update_layout(**CL, height=220,
        title=dict(text="Phương thức thanh toán", font=dict(size=13,color=TEXT), x=0),
        legend=dict(font=dict(color=SUBTEXT,size=12), bgcolor="rgba(0,0,0,0)", orientation="h", yanchor="bottom", y=-0.15),
        showlegend=True,
    )
    st.plotly_chart(fig_pt, use_container_width=True)
    st.markdown(f"""
    <div style='font-size:12px;color:{SUBTEXT};line-height:2;margin-top:4px'>
...
st.dataframe(df_det, use_container_width=True, hide_index=True,
    column_config={"Ngày": st.column_config.TextColumn(width=110), "Doanh thu": st.column_config.TextColumn(width=130)})
